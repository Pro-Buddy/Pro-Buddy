...
export default function EvaluateSection() {
    ...
    function handleSubmit(e) {
        ...
        .then(res => {
            if (!res.success) throw new Error(res.message || 'Evaluation failed');
            setResult(res);
            setFormData(fd);
            setShowForm(false);
            setLoading(false);

            // Save to backend & local backup
            if (user) {
                const evalEntry = {
                    ...
                    title: fd.title,
                    domain: fd.domain,
                    overallScore: res.scores.overall,
                    scores: res.scores,
                    verdict: res.feedback.verdict
                };
                apiSaveEvaluation(evalEntry).catch(() => {});
                try {
                    const localHistory = JSON.parse(localStorage.getItem(`probuddy_history_${user.email}`) || '[]');
                    localHistory.unshift(evalEntry);
                    localStorage.setItem(`probuddy_history_${user.email}`, JSON.stringify(localHistory.slice(0, 50)));
                } catch {
        ...
        .catch(err => {
            console.warn("Real-time Gemini analysis failed, falling back to offline evaluator:", err);
        ...
        <section id="evaluate" className="evaluate-section">
            <div className="section-container">
                <div className="section-header">
                    <h2 className="section-title">Pro <span className="gradient-text">Evaluator</span></h2>
                    ...
                    {!showForm && result && (
                        <div className="eval-results-container" id="evalResultsContainer" ref={resultsRef}>
                            ...
                            <div className="dimension-scores" id="dimensionScores">
                                ...
                                <DimensionCard id="dimTechStack" icon="" name="Tech Stack Required" score={result.scores.techStack} feedback={result.feedback.dimFeedback.techStack} delay={600} />
                            </div>
                            <div className="insights-panel" id="insightsPanel">
                                ...
                                <div className="insight-section"><h4> Areas for Improvement</h4><ul>{result.feedback.weaknesses.map((w, i) => <li key={i}>{w}</li>)}</ul></div>
                            ...
                            <div className="results-actions">
                                ...
                                {result.scores.overall > 75 && (
                                    <a 
                                        href={`https://angel.co/search?q=investors%20${encodeURIComponent(result.domainData.name)}%20startup`} 
                                        target="_blank" 
                            ...
                            {result.scores.overall > 75 && (
                                <p className="investor-invite-text">
                                     <strong>Investment Ready!</strong> Your project score is top-tier. Use the button above to start connecting with domain-specific funders and companies.
                                </p>
                            )}
                        </div>
                    )}
