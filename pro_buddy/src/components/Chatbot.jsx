...
export default function Chatbot({ currentView }) {
    ...
    return (
        <>
            <button className={`chatbot-toggle ${imageError ? 'fallback-mode' : ''}`} onClick={() => setIsOpen(!isOpen)}>
                {isOpen ? (
                    <X size={28} color={imageError ? "#fff" : "currentColor"} />
                ) : imageError ? (
                    <Bot size={32} color="#fff" className="chatbot-icon-anim" />
                ) : (
                    <img 
                        src="/image.png" 
                        alt="AI Assistant" 
