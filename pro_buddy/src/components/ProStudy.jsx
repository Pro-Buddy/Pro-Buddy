    ...
    async function search() {
        ...
        try {
            ...
            if (data.success) {
                ...
                showToast(' Failed to fetch study resources: ' + data.message);
            }
        } catch (err) {
        ...
        <section className="study-section" id="proStudy" style={{ paddingTop: '20px' }}>
            ...
            <div className="section-container">
                <div className="section-header">
                    <h2 className="section-title">Pro <span style={{ color: '#E64A19' }}>Study</span></h2>
                ...
                {showForm ? (
                    <div className="study-form-container eval-form-container">
                        ...
                        <button type="button" className="btn btn-primary btn-lg btn-full" style={{ marginTop: '24px' }} onClick={search} disabled={isSearching}>
                            {isSearching ? 'Curating Resources with AI...' : 'Find Best Resources'}
                        </button>
                    </div>
                ) : result && (
                        ...
                        <div className="study-section-block">
                            ...
                            <div className="study-yt-grid">
                                {result.youtube.map((yt, i) => (
                                    <a href={yt.url} target="_blank" rel="noopener noreferrer" className="study-yt-card" key={i} style={{ textDecoration: 'none', display: 'flex', gap: '16px', alignItems: 'center' }}>
                                        <div className="study-yt-icon" style={{ fontSize: '24px' }}></div>
                                        <div>
                                            <h5 className="study-yt-channel" style={{ margin: '0 0 4px 0', color: 'var(--text-primary)', fontSize: '0.9rem' }}>{yt.channel}</h5>
                                            <p className="study-yt-title" style={{ margin: '0 0 6px 0', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>{yt.title}</p>
                                            <span className="study-yt-lang" style={{ background: 'var(--bg-tertiary)', padding: '2px 8px', borderRadius: '12px', fontSize: '0.75rem', color: 'var(--text-muted)' }}>{yt.lang}</span>
                                        </div>
                                    </a>
                                ))}
                            </div>
                        ...
                        <div className="study-section-block">
                            ...
                            <div className="study-web-grid">
                                {result.websites.map((w, i) => (
                                    <a href={w.url} target="_blank" rel="noopener noreferrer" className="study-web-card" key={i} style={{ textDecoration: 'none', display: 'block' }}>
                                        <h5 className="study-web-name" style={{ margin: '0 0 6px 0', color: 'var(--text-primary)', fontSize: '0.95rem' }}>{w.name}</h5>
                                        <p className="study-web-desc" style={{ margin: '0', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>{w.desc}</p>
                                    </a>
                                ))}
                            </div>
                        </div>
