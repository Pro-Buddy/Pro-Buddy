...
async function safeGenerateContent(prompt, schema) {
    ...
    // 1. Try Ollama First
    try {
        console.log("[LLM Service] Attempting Ollama first...");
        return await callOllama(prompt, schema);
    } catch (err) {
        console.warn("[LLM Service] Ollama failed:", err.message);
        ollamaError = err;
    }
    ...
    if (ai) {
        console.log("[LLM Service] Ollama failed, attempting Gemini fallback...");
        const MAX_RETRIES = 3;
        ...
        console.warn("[LLM Service] Gemini API not initialized. Cannot fallback to Gemini.");
    }

    // 3. Manual Fallback (Let server.js handle it)
    console.warn("[LLM Service] Both Ollama and Gemini failed. Throwing error to trigger service-specific fallbacks.");
    throw geminiError || ollamaError || new Error("All LLM services failed.");
}
...
Act as an expert career counselor. Suggest EXACTLY 5 highly relevant, specific job roles. For each role, provide a realistic company that hires for this role, and a realistic salary range.
CRITICAL: Do NOT use or link to LinkedIn, Glassdoor, or any other mainstream generic job board. Instead, provide a direct job description search URL for this job role on one of the following remote job boards:
remote.co, nodesk.co, workster.co, workew.com, flexjobs.com, remoters.net, pangian.com, remotive.com, justremote.co, jobspresso.co, jobgether.com, wellfound.com, remoteok.com, powertofly.com, skipthedrive.com, dynamitejobs.com, citizenremote.com, euremotejobs.com, workingnomads.com, virtualvocations.com, weworkremotely.com, inclusivelyremote.com, remotenomadjobs.com, opentoworkremote.com, remotehealthcarejobs.com.

Also suggest EXACTLY 5 platforms or websites where they can explore more about these careers (must directly link to their JDs / job listings).
...
Then, curate a list of EXACTLY 5 high-quality YouTube playlists (these MUST be direct playlist URLs, e.g., containing list= or direct video series, NOT search results) that are widely recognized as the best resources to learn this. Also specify the language of the tutorial (e.g. English, Hindi).
Then, curate a list of EXACTLY 5 high-quality websites, documentations, or platforms (like official docs, GeeksforGeeks, MDN, etc.) to learn this topic, with a short description of what they offer. Ensure the URLs are valid generic paths.
Return a structured JSON object.`;
    const schema = {
        type: Type.OBJECT,
        properties: {
...
4. Project Evaluator: Enter Project Title, Problem, Solution, Domain, Team Size, Tech Stack, Audience, USP. It evaluates on 5 dimensions (out of 20 each): Difficulty, Success Probability, Uniqueness, Tech Knowledge, Tech Stack Alignment. Overall score is out of 100. >75 is excellent (shows "Connect with Investors" button), <55 means needs major work. Returns strengths, weaknesses, differentiation, and next steps. You can download the report.
5. Pro Careers: Input skills, branch, interest, location. AI matches you with 5 roles (shows title, company, est. salary, skills) and gives direct job board links (no LinkedIn). You can search for internships or remote jobs by typing that in "interest".
6. Pro Study: Input ANY topic (Web Dev, ML, System Design, etc.). AI curates 5 working YouTube playlist links (in Hindi or English) and 5 official website docs (like MDN). 
