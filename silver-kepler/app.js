...
window.addEventListener('scroll', () => {
    ...
    // Active nav link
    const sections = ['home', 'features', 'how-it-works', 'evaluate'];
    let current = '';
    sections.forEach(id => {
...
function scrollToSection(id) {
    // If profile is visible, switch back to main sections first
...
document.getElementById('navCtaBtn').addEventListener('click', () => scrollToSection('evaluate'));

// Make all nav links also trigger showMainSections when clicked
document.querySelectorAll('.nav-link').forEach(link => {
...
const DOMAIN_DATA = {
    ...
    'other': {
        ...
        keywords: [],
        techFeasibilityBase: 70,
        innovationBase: 60
    }
};

const TIMELINE_MULTIPLIERS = {
    '24h': { difficultyModifier: 10, successModifier: -15 },
    '48h': { difficultyModifier: 5, successModifier: -5 },
    '1week': { difficultyModifier: 0, successModifier: 5 },
    '1month': { difficultyModifier: -5, successModifier: 10 },
    'semester': { difficultyModifier: -10, successModifier: 15 }
};

const TEAM_MULTIPLIERS = {
    '1': { difficultyModifier: 10, successModifier: -10 },
    '2': { difficultyModifier: 3, successModifier: -3 },
    '3': { difficultyModifier: 0, successModifier: 0 },
    '5': { difficultyModifier: -5, successModifier: 5 }
};
...
// ---- Sub-parameter Scoring Helpers ----

function calculateDifficulty(formData, allText, stackDifficultyAvg, stackCount) {
    let score = 50;

    // 1. Number of features/modules (detected by commas or bullet points in solution)
    const featureCount = formData.solution.split(/,|\n|•|- /).length;
    score += (featureCount > 5 ? 10 : featureCount * 2);

    // 2. Technology complexity
    score += (stackDifficultyAvg - 1.5) * 20; // +10 for hard, -10 for easy

    // 3. Integration level
    const integrations = countKeywords(allText, ['api', 'integrate', 'connect', 'third-party', 'service', 'oauth', 'payment']);
    score += integrations * 5;

    // 4. Learning required
    const learningKeywords = countKeywords(allText, ['learn', 'study', 'research', 'new to', 'unfamiliar', 'custom']);
    score += learningKeywords * 4;

    // 5. Dependency level
    const dependencies = countKeywords(allText, ['hardware', 'sensor', 'gps', 'camera', 'bluetooth', 'external', 'device']);
    score += dependencies * 6;
...
function calculateSuccess(formData, allText, teamSize, stackCount) {
    ...
    // 1. Scope vs Time (fixed 48h for now, so heavily penalize large scope)
    ...
    // 3. Data/Resource Availability
    const resources = countKeywords(allText, ['dataset', 'data', 'open source', 'api', 'library', 'framework', 'available']);
    score += resources * 4;

    // 4. Demo Feasibility
    const demoKeywords = countKeywords(allText, ['demo', 'ui', 'frontend', 'visual', 'prototype', 'mockup', 'screen']);
    score += demoKeywords * 5;

    // 5. Risk Count (Integration/Deployment)
    const risks = countKeywords(allText, ['security', 'privacy', 'payment', 'real-time', 'hardware', 'complex']);
    score -= risks * 4;

    return clamp(score, 10, 98);
}

function calculateUniqueness(formData, allText, domainRelevance) {
    let score = 50;

    // 1. Problem Originality
    if (allText.includes('daily') || allText.includes('common') || allText.includes('student')) score -= 5; // Common problems
    ...
    const novelty = countKeywords(allText, ['novel', 'new', 'first', 'patent', 'invention', 'innovative', 'unique']);
    score += novelty * 5;
    ...
    // 5. Existing Solution Similarity
    // (Simulated: if they mention existing solutions, they are aware, so we punish less. If vague, punish more)
...
function calculateTechKnowledge(formData, allText, stackCount, requiredSkills) {
    ...
    // 1. Required Prog Level
    if (stackCount > 3) score += 10;
    if (allText.includes('advanced') || allText.includes('senior')) score += 10;

    // 2. Domain Knowledge
    const domainTerms = countKeywords(allText, ['industry', 'market', 'regulations', 'compliance', 'process', 'workflow']);
    score += domainTerms * 4;

    // 3. Algorithm Complexity
    const algoTerms = countKeywords(allText, ['algorithm', 'optimize', 'search', 'sort', 'graph', 'tree', 'ai model']);
    score += algoTerms * 6;
    ...
    // 5. System Design
    const systemTerms = countKeywords(allText, ['architecture', 'database schema', 'flow', 'component', 'system', 'structure']);
    score += systemTerms * 5;
...
function calculateTechStackReq(formData, allText, stackCount) {
    ...
    const frontend = countKeywords(formData.techStack.toLowerCase(), ['react', 'vue', 'angular', 'html', 'css', 'ui', 'flutter', 'swift']);
    score += frontend * 5;

    // 2. Backend Complexity
    const backend = countKeywords(formData.techStack.toLowerCase(), ['node', 'express', 'python', 'java', 'go', 'php', 'server']);
    score += backend * 5;

    // 3. Database Level
    const db = countKeywords(formData.techStack.toLowerCase(), ['mongo', 'sql', 'firebase', 'data', 'storage']);
    score += db * 5;

    // 4. Advanced Tech
    const adv = countKeywords(allText, ['ai', 'ml', 'blockchain', 'iot', 'ar', 'vr', 'cloud']);
    score += adv * 8;

    // 5. Deployment
    const deploy = countKeywords(allText, ['aws', 'cloud', 'hosting', 'deploy', 'docker', 'ci/cd']);
    score += deploy * 6;
...
// ---- Evaluation Logic ----
function evaluateIdea(formData) {
    ...
    const uniqueness = calculateUniqueness(formData, allText, 50); // simplified domain relevance
    ...
    feedback.conclusion = conclusion; // Append conclusion
...
function generateFeedback(scores, formData, domainData, detectedTechs, requiredSkills) {
    ...
    // Dimension feedback (simplified for now)
    const dimFeedback = {
...
document.getElementById('evalForm').addEventListener('submit', function (e) {
    ...
    // Field Validation
    const reqIds = ['projectTitle', 'problemStatement', 'proposedSolution', 'techStack', 'targetAudience', 'uniqueFeatures'];
    let emptyField = null;
    ...
    if (emptyField) {
        showToast('⚠️ Please fill in all required fields to proceed.', 'error');
        return; // Stop execution
    }

    const formData = {
        title: document.getElementById('projectTitle').value.trim(),
        problem: document.getElementById('problemStatement').value.trim(),
        solution: document.getElementById('proposedSolution').value.trim(),
        domain: document.getElementById('domain').value,
        teamSize: document.getElementById('teamSize').value,
        techStack: document.getElementById('techStack').value.trim(),
        timeline: '48h',
        targetAudience: document.getElementById('targetAudience').value.trim(),
        uniqueFeatures: document.getElementById('uniqueFeatures').value.trim()
    };
    ...
    setTimeout(() => {
        const result = evaluateIdea(formData);
...
function displayResults(result, formData) {
    const { scores, feedback, domainData } = result;

    // Hide form, show results
    document.getElementById('evalFormContainer').style.display = 'none';
    const resultsContainer = document.getElementById('evalResultsContainer');
    resultsContainer.style.display = 'block';

    // Project title
    document.getElementById('resultsProjectTitle').textContent = `📊 ${formData.title}`;

    // Conclusion
    const conclusionEl = document.getElementById('finalConclusionText');
    if (conclusionEl) conclusionEl.textContent = feedback.conclusion;

    // Overall score ring (SVG gradient workaround)
    ...
    const ringProgress = document.getElementById('overallRingProgress');
    ...
    // Verdict
    document.getElementById('overallVerdict').textContent = feedback.verdict;
    const verdictTagsEl = document.getElementById('verdictTags');
    verdictTagsEl.innerHTML = '';
    feedback.verdictTags.forEach(tag => {
        const span = document.createElement('span');
        span.className = `verdict-tag tag-${tag.type}`;
        span.textContent = tag.text;
        verdictTagsEl.appendChild(span);
    });

    // Dimension scores
    const dimensions = [
        { id: 'dimDifficulty', key: 'difficulty' },
        { id: 'dimSuccess', key: 'success' },
        { id: 'dimUniqueness', key: 'uniqueness' },
        { id: 'dimTechKnowledge', key: 'techKnowledge' },
        { id: 'dimTechStack', key: 'techStack' }
    ];

    dimensions.forEach((dim, i) => {
        const card = document.getElementById(dim.id);
        const score = scores[dim.key];
        const scoreEl = card.querySelector('.dim-score');
        const fillEl = card.querySelector('.dim-fill');
        const feedbackEl = card.querySelector('.dim-feedback');

        scoreEl.textContent = `${score}%`;

        // Set color class
        fillEl.className = 'dim-fill';
        if (score >= 75) fillEl.classList.add('high');
        else if (score >= 55) fillEl.classList.add('medium');
        ...
        feedbackEl.textContent = feedback.dimFeedback[dim.key];
    });

    // Lists
    updateList('strengthsList', feedback.strengths);
    updateList('weaknessesList', feedback.weaknesses);
    updateList('suggestionsList', feedback.suggestions);
    updateList('nextStepsList', feedback.nextSteps);

    // Existing Solutions (AI-generated)
    updateExistingSolutions(domainData.existingSolutions);

    // Industry Cards
    const industryCards = document.getElementById('industryCards');
    industryCards.innerHTML = '';
    domainData.industries.forEach(ind => {
        const card = document.createElement('div');
        card.className = 'industry-card';
        card.innerHTML = `
            <div class="industry-card-name">${ind.name}</div>
            <div class="industry-card-desc">${ind.desc}</div>
            <div class="industry-card-companies">
                ${ind.companies.map(c => `<span class="company-tag">${c}</span>`).join('')}
            </div>
        `;
        industryCards.appendChild(card);
    });

    // Store results for download/share
    window.__probuddyResults = { scores, feedback, formData, domainData };

    // Save to user's evaluation history
    if (typeof saveEvaluation === 'function') {
        saveEvaluation(formData, scores, feedback);
    }
...
function resetEvaluation() {
    ...
    document.getElementById('evalResultsContainer').style.display = 'none';
    ...
    document.getElementById('evalFormContainer').scrollIntoView({ behavior: 'smooth', block: 'start' });
}

// ---- Download Report ----
function downloadReport() {
    const { scores, feedback, formData, domainData } = window.__probuddyResults;

    let report = `
╔══════════════════════════════════════════════════╗
║           PRO BUDDY — PROJECT ANALYSIS REPORT           ║
║              Confusion to Execution                      ║
╚══════════════════════════════════════════════════╝
...
function shareResults() {
    const { scores, formData } = window.__probuddyResults;
...
function updateExistingSolutions(solutions) {
    const list = document.getElementById('existingSolutionsList');
    if (!list) return;
    list.innerHTML = '';
    if (solutions && solutions.length > 0) {
        solutions.forEach(sol => {
            const li = document.createElement('li');
            li.innerHTML = `<strong>${sol.name}</strong> — ${sol.desc} <span style="color: var(--accent-info); font-size: 0.78rem;">(${sol.url})</span>`;
            list.appendChild(li);
        });
    } else {
        list.innerHTML = '<li>No specific existing solutions found for this domain yet.</li>';
    }
}
